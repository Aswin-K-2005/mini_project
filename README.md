...
- If audio is crackling under heavy load, try `--blur-mode pixelation` and `--stt-model-size tiny`; latest build decouples audio output from video rendering and adds underrun counters to reduce crackle when FPS drops.

- Timing alignment tip: start with `--stt-alignment-delay-ms 180` to `260` so beep events arrive before playback without excessive delay; increase only if beeps still miss under load.
