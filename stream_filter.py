...
class FilterConfig:
    ...
    stt_alignment_delay_ms: float = 220.0
...
class StreamingFilterProduction:
    def __init__(self, config: FilterConfig):
        ...
        self.audio_play_queue = Queue(maxsize=96)
        ...
        self.stats = {
            ...
            'runtime_errors': 0,
            'beep_events_scheduled': 0,
            'beep_events_applied': 0,
            'beep_events_missed': 0,
            'audio_underflows': 0,
                    ...
                    for idx, censored_seg in enumerate(censored_segments):
                        ...
                        for word_event in word_events:
                            ...
                            if ev_end > now_cutoff:
                                ...
                                self._safe_stat_inc('beep_events_scheduled')
                                hit_count += 1
    ...
    def _apply_scheduled_beeps(self, audio_chunk, chunk_ts):
        ...
        with self.beep_lock:
            while self.beep_events and self.beep_events[0][1] < chunk_ts - 0.3:
                ...
                self._safe_stat_inc('beep_events_missed')
        ...
        for ev_start, ev_end in events_snapshot:
            ...
            applied_any = True
        if applied_any:
            self._safe_stat_inc('beep_events_applied')
        return out
        ...
        try:
            while not self.stop_event.is_set():
                ...
                while self.pending_audio and (now_mono - self.pending_audio[0]['ts']) >= hold_s:
                    ready_pkt = self.pending_audio.popleft()
                    self._bounded_put(self.audio_queue, ready_pkt, 'drops_audio_queue')
                    self._bounded_put(self.audio_play_queue, ready_pkt, 'drops_audio_queue')
        finally:
    ...
    def _sync_thread(self):
        """Video-first sync path: keep frames flowing and let audio run on dedicated queue."""
        while not self.stop_event.is_set():
            try:
            ...
            pkt = AVPacket(video_pkt['frame'], np.zeros(self.audio_chunk, dtype=np.float32), v_ts, video_pkt.get('metadata', {}))
    ...
    def _audio_output_thread(self):
        ...
        chunk_duration_s = self.audio_chunk / float(self.audio_rate)
        try:
            while not self.stop_event.is_set():
                try:
                    pkt = self.audio_play_queue.get(timeout=chunk_duration_s)
                    audio_chunk = pkt['audio'].astype(np.float32)
                except Empty:
                    audio_chunk = silence
                    self._safe_stat_inc('audio_underflows')
    ...
    def _draw_stats_overlay(self, frame):
        ...
        lines = [
            ...
            f"Beeps sch/app/miss: {s['beep_events_scheduled']}/{s['beep_events_applied']}/{s['beep_events_missed']}",
            f"Audio underflows: {s['audio_underflows']}",
    ...
    def start(self):
        ...
        self.threads = [
            ...
            threading.Thread(target=self._audio_output_thread, daemon=True),
...
def parse_args():
    ...
    parser.add_argument('--stt-alignment-delay-ms', type=float, default=220.0)
